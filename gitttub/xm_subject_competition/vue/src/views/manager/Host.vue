<template>
  <div>
    ...
    <el-dialog title="主办方信息" v-model="data.formVisible" width="40%" destroy-on-close>
...
const add = () => {
  request.post('/host/add', data.form).then(res => {
    if (res.code === '200') {
...
const update = () => {
  request.put('/host/update', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      data.formVisible = false
      load()
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/host/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete("/host/delete/batch", {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()
      } else {
