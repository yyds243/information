  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="hostName" label="主办方名称"></el-table-column>
    ...
    <el-dialog title="奖项信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" :rules="data.rules" label-width="80px"  style="padding: 20px 30px" ref="formRef">
        <el-form-item label="赛事" prop="eventId">
          <el-select style="width: 100%" v-model="data.form.eventId">
            <el-option v-for="item in data.eventList" :key="item.id" :value="item.id" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="奖金" prop="money">
          <el-input-number :min="0" style="width: 200px" v-model="data.form.money" placeholder="奖金"></el-input-number>
        </el-form-item>
      </el-form>
      <template #footer>
...
const add = () => {
  ...
  request.post('/award/add', data.form).then(res => {
    if (res.code === '200') {
...
// 更新
const update = () => {
  request.put('/award/update', data.form).then(res => {
    if (res.code === '200') {
      data.formVisible = false
      load()
    } else {
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/award/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/award/delete/batch', {data: data.ids}).then(res => {
      if (res.code === '200') {
